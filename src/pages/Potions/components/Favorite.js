...
import {
  addPotionFavorite,
  removePotionFavorite,
} from "../../../store/potionFavoriterSlice";
import { useDispatch, useSelector } from "react-redux";
import { toast } from "react-toastify";

function Favorite({ data }) {
  const favorites = useSelector((state) => state.potionFavoriterStore.potions);
  ...
  function favoriteClickHandler() {
    if (isFavorite) {
      setIsFavorite(false);
      dispatch(removePotionFavorite(data));
      ...
      if (favorites.length > 2) {
        ...
        setIsFavorite(true);
        dispatch(addPotionFavorite(data));
