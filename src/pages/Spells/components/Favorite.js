...
import {
  addSpellFavorite,
  removeSpellFavorite,
} from "../../../store/spellFavoriterSlice";
import { useDispatch, useSelector } from "react-redux";
import { toast } from "react-toastify";

function Favorite({ data }) {
  const favorites = useSelector((state) => state.spellFavoriterStore.spells);
  ...
  function favoriteClickHandler() {
    if (isFavorite) {
      setIsFavorite(false);
      dispatch(removeSpellFavorite(data));
      ...
      if (favorites.length > 2) {
        ...
        setIsFavorite(true);
        dispatch(addSpellFavorite(data));
